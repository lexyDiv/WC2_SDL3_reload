...
void Console::proc(float mX, float mY, bool pressed)
{
...
void Console::draw()
{
    ...
    if (this->strArr.length)
    {
        ...
        if (l >= 12)
        {
         ...
         ctx.FillRect(this->x, this->y, this->width, this->height, "yellow", A);
        if (this->clicked)
        {
        ...
        int iter = 0;
        int delta = this->strArr.length > 12 ? 12 : this->strArr.length;
...
void Console::drawSB(int A)
{
    ...
    SDL_FPoint sp = {.x = this->scrollRunner.x, .y = this->scrollRunner.y};
    SDL_FPoint fp = {.x = this->scrollRunner.x + 30, .y = this->scrollRunner.y};
